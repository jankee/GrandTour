    ...
    public class MyCustomCollectionTriggerer : MonoBehaviour, IInventoryItemContainer
    {
        ...
        public void Awake()
        {
            ...
            triggerer.OnTriggerUse += (player) =>
            {
                ...
                collection.SyncActions(items); // All actions done in the UI window should be synced to this item array.
            ...
            {
                collection.StopSyncActions(items); // Stop syncing actions
            };
        }
    }
}