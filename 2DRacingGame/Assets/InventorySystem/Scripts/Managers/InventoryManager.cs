...
//                }
        ...
        /// <returns></returns>
        public static bool AddItems(IEnumerable<InventoryItemBase> items, ICollection<InventoryItemBase> storedItems = null, bool repaint = true)
        {
            if (CanAddItems(items.ToArray()) == false)
            {
                return false;
            }
        ...
        {
            return CanAddItemCount(row.item) >= row.amount;
        }

        public static bool CanAddItems(InventoryItemAmountRow[] items)
        {
            _counter.LoadFrom(_lootToCollections.ToArray());
        ...
        public static bool CanAddItems(InventoryItemBase[] items)
        {
            return CanAddItems(InventoryItemUtility.ItemsToRows(items));
        }
