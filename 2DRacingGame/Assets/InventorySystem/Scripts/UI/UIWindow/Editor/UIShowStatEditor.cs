...
namespace Devdog.InventorySystem.Editors
{
    ...
    public class UIShowStatEditor : Editor
    {
        ...
        private Dictionary<string, List<IInventoryCharacterStat>> stats = new Dictionary<string, List<IInventoryCharacterStat>>();
        ...
        private void UpdateStats()
        {
            ...
            if (p.Length > 0)
            {
                var itemDatabase = ItemManager.database;
                ...
                foreach (var property in itemDatabase.properties)
                {
                    ...
                    if (stats.ContainsKey(property.category) == false)
                        stats.Add(property.category, new List<IInventoryCharacterStat>());
                ...
                foreach (var equipStat in itemDatabase.equipStats)
                {
                    if (stats.ContainsKey(equipStat.category) == false)
                        stats.Add(equipStat.category, new List<IInventoryCharacterStat>());
