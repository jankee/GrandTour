...
//            }
        ...
        protected virtual bool GiveCraftReward(InventoryCraftingCategory category, InventoryCraftingBlueprint blueprint)
        {
            ...
            {
                // Store crafted item
                var itemsToAdd = new InventoryItemBase[blueprint.resultItems.Length];
                for (int i = 0; i < itemsToAdd.Length; i++)
                {
                    var c = GameObject.Instantiate<InventoryItemBase>(blueprint.resultItems[i].item);
                    c.currentStackSize = blueprint.resultItems[i].amount;
                    itemsToAdd[i] = c;
                }
